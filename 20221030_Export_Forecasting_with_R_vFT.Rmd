...
tswcv <- function(series, x, y, model, min.window.size, window.type=c("fixed", "variable")){
  ...
  k <- dim(series)[1]-min.window.size-1
  ...
  for (i in 1:k){
    ...
    windows.size <- c(windows.size, n-m)
  }
...
rmse_fit6_fw <- tswcv(series=midwest_soybean, x="ref_date_fmd", y="vl_mm_ton", model=fit6, min.window.size=20, window.type="fixed")
cv_table <- bind_rows(cv_table, tibble(model="Stepwise: SARIMA(0,0,2)(1,1,2)[12]",lambda=fit6$lambda[1], rmse=rmse_fit6_fw$rmse))

rmse_fit6_vw <- tswcv(series=midwest_soybean, x="ref_date_fmd", y="vl_mm_ton", model=fit6, min.window.size=20, window.type="variable")
cv_table <- bind_rows(cv_table, tibble(model="Stepwise: SARIMA(0,0,2)(1,1,2)[12]",lambda=fit6$lambda[1], rmse=rmse_fit6_vw$rmse))

rmse_fit12_fw <- tswcv(series=midwest_soybean, x="ref_date_fmd", y="vl_mm_ton", model=fit12, min.window.size=20, window.type="fixed")
cv_table <- bind_rows(cv_table, tibble(model="Stepwise: SARIMA(3,0,0)(0,1,1)[12]",lambda=fit12$lambda[1], rmse=rmse_fit12_fw$rmse))

rmse_fit12_vw <- tswcv(series=midwest_soybean, x="ref_date_fmd", y="vl_mm_ton", model=fit12, min.window.size=20, window.type="variable")
cv_table <- bind_rows(cv_table, tibble(model="Stepwise: SARIMA(3,0,0)(0,1,1)[12]",lambda=fit12$lambda[1], rmse=rmse_fit12_vw$rmse))
